...
const ALL_IMAGES = Array.from({ length: 28 }, (_, i) => ({
  id: i,
  src: `/gojo-wallpapers/gojo-wallpaper${i + 1}.png`,
  alt: `Gojo Satoru Wallpaper ${i + 1} - Jujutsu Kaisen HD Phone Background`,
}));

export default function GojoGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `gojo-wallpaper-${idx + 1}.png`;
      a.click();
