...
const ALL_IMAGES = Array.from({ length: 32 }, (_, i) => ({
  id: i,
  src: `/goku-wallpapers/goku-wallpaper${i + 1}.png`,
  alt: `Goku Wallpaper ${i + 1} - Dragon Ball Super Saiyan HD Phone Background`,
}));

export default function GokuGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `goku-wallpaper-${idx + 1}.png`;
      a.click();
