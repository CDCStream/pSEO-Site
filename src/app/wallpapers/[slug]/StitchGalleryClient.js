...
const ALL_IMAGES = Array.from({ length: 15 }, (_, i) => ({
  id: i,
  src: `/stitch-wallpaper/stitch%20wallpaper${i + 1}.png`,
  alt: `Stitch Wallpaper ${i + 1} - Cute Lilo & Stitch Phone Background HD`,
}));

export default function StitchGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `stitch-wallpaper-${idx + 1}.png`;
      a.click();
    ...
    <>
      <div className="bg-white/5 rounded-2xl border border-white/10 p-4 sm:p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-white">{ALL_IMAGES.length} Stitch Wallpapers</h2>
            ...
            <div key={img.id} className="group relative rounded-xl overflow-hidden border border-white/10 bg-black/20 aspect-[9/16] cursor-pointer" onClick={() => openLightbox(i)}>
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-between p-3">
                ...
                <button
                  ...
                  className="p-2 rounded-lg bg-white/20 backdrop-blur-sm hover:bg-blue-500/80 text-white transition-colors"
                  aria-label={`Download Stitch Wallpaper ${i + 1}`}
                >
          ...
          <div className="max-w-4xl max-h-[85vh] flex flex-col items-center gap-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-blue-500 to-cyan-500 text-white font-semibold hover:opacity-90 transition-opacity inline-flex items-center gap-2"
              >
                <Download className="w-4 h-4" />
                Download HD
              </button>
