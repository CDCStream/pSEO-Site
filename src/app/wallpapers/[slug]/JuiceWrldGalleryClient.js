...
const ALL_IMAGES = Array.from({ length: 32 }, (_, i) => ({
  id: i,
  src: `/juice-wrld-wallpapers/juice-wrld-wallpaper${i + 1}.png`,
  alt: `Juice WRLD Wallpaper ${i + 1} - Free HD 999 Phone Background`,
}));

export default function JuiceWrldGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `juice-wrld-wallpaper-${idx + 1}.png`;
      a.click();
        ...
        {ALL_IMAGES.map((img, i) => (
          ...
          >
            ...
            <button
              ...
              className="absolute bottom-2 right-2 p-2 bg-purple-600 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity hover:bg-purple-500"
              aria-label={`Download Juice WRLD Wallpaper ${i + 1}`}
            >
      ...
      {lightbox !== null && (
        <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center" onClick={closeLightbox}>
          ...
          <button
            ...
            className="absolute bottom-6 px-6 py-3 bg-purple-600 hover:bg-purple-500 rounded-xl text-white font-medium flex items-center gap-2 transition-colors"
          >
