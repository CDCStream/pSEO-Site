...
const ALL_IMAGES = Array.from({ length: 32 }, (_, i) => ({
  id: i,
  src: `/summer-wallpaper/summer-wallpaper${i + 1}.png`,
  alt: `Summer Wallpaper ${i + 1} - Beach & Tropical Phone Background HD`,
}));

export default function SummerGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `summer-wallpaper-${idx + 1}.png`;
      a.click();
