...
const ALL_IMAGES = Array.from({ length: 28 }, (_, i) => ({
  id: i,
  src: `/peppa-pig-house-wallpapers/peppa-pig-house-wallpaper${i + 1}.png`,
  alt: `Peppa Pig House Wallpaper ${i + 1} - Cute Red House on the Hill Phone Background HD`,
}));

export default function PeppaPigHouseGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `peppa-pig-house-wallpaper-${idx + 1}.png`;
      a.click();
