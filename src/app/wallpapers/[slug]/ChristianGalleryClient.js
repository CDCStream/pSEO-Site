...
const ALL_IMAGES = Array.from({ length: 32 }, (_, i) => ({
  id: i,
  src: `/christian-wallpapers/christian-wallpaper${i + 1}.png`,
  alt: `Christian Wallpaper ${i + 1} - Bible Verse Cross Faith HD Phone Background`,
}));

export default function ChristianGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `christian-wallpaper-${idx + 1}.png`;
      a.click();
