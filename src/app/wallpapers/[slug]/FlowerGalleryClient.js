...
const ALL_IMAGES = Array.from({ length: 32 }, (_, i) => ({
  id: i,
  src: `/flower-wallpapers/flower-wallpaper${i + 1}.png`,
  alt: `Flower Wallpaper ${i + 1} - Beautiful Floral Phone Background HD`,
}));

export default function FlowerGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `flower-wallpaper-${idx + 1}.png`;
      a.click();
