...
const ALL_IMAGES = Array.from({ length: 32 }, (_, i) => ({
  id: i,
  src: `/white-wallpapers/white-wallpaper${i + 1}.png`,
  alt: `White Wallpaper ${i + 1} - White Aesthetic Phone Background HD`,
}));
...
export default function WhiteGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `white-wallpaper-${idx + 1}.png`;
      a.click();
        ...
        {ALL_IMAGES.map((img, i) => (
          ...
          >
            ...
            <button
              ...
              className="absolute bottom-2 right-2 p-2 bg-slate-500/90 hover:bg-slate-400 text-white rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-300 backdrop-blur-sm"
        ...
        <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-sm flex items-center justify-center p-4" onClick={closeLightbox}>
          ...
          <div className="flex flex-col items-center gap-4" onClick={(e) => e.stopPropagation()}>
            ...
            <button
              ...
              className="flex items-center gap-2 px-6 py-3 bg-slate-500 hover:bg-slate-400 text-white rounded-xl font-semibold transition-colors"
            >
