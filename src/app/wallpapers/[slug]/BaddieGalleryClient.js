...
const ALL_IMAGES = Array.from({ length: 32 }, (_, i) => ({
  id: i,
  src: `/baddie-wallpapers/baddie-wallpaper${i + 1}.png`,
  alt: `Baddie Wallpaper ${i + 1} - Aesthetic Bad Girl Y2K Glam HD Phone Background for iPhone & Android`,
}));

export default function BaddieGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `baddie-wallpaper-${idx + 1}.png`;
      a.click();
