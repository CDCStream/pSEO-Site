...
const ALL_IMAGES = Array.from({ length: 32 }, (_, i) => ({
  id: i,
  src: `/katseye-wallpapers/katseye-wallpaper${i + 1}.png`,
  alt: `KATSEYE Wallpaper ${i + 1} - Global K-Pop Girl Group HD Phone Background`,
}));

export default function KatseyeGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `katseye-wallpaper-${idx + 1}.png`;
      a.click();
