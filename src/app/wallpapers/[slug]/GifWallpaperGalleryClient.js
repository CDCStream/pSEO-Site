...
const ALL_IMAGES = Array.from({ length: 30 }, (_, i) => ({
  id: i,
  src: `/gif-wallpapers/gif-wallpaper${i + 1}.png`,
  alt: `GIF Wallpaper ${i + 1} - Animated Style Phone Background HD`,
}));

export default function GifWallpaperGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `gif-wallpaper-${idx + 1}.png`;
      a.click();
