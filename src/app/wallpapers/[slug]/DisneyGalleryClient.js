...
const ALL_IMAGES = Array.from({ length: 28 }, (_, i) => ({
  id: i,
  src: `/disney-wallpapers/disney-wallpaper${i + 1}.png`,
  alt: `Disney Wallpaper ${i + 1} - Free HD Phone Background`,
}));

export default function DisneyGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `disney-wallpaper-${idx + 1}.png`;
      a.click();
        ...
        {ALL_IMAGES.map((img, i) => (
          ...
          >
            ...
            <button
              ...
              className="absolute bottom-2 right-2 p-2 bg-blue-500 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity hover:bg-blue-400"
              aria-label={`Download Disney Wallpaper ${i + 1}`}
            >
      ...
      {lightbox !== null && (
        <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center" onClick={closeLightbox}>
          ...
          <button
            ...
            className="absolute bottom-6 px-6 py-3 bg-blue-500 hover:bg-blue-400 rounded-xl text-white font-medium flex items-center gap-2 transition-colors"
          >
