...
const ALL_IMAGES = Array.from({ length: 32 }, (_, i) => ({
  id: i,
  src: `/cat-wallpapers/cat-wallpaper${i + 1}.png`,
  alt: `Cat Wallpaper ${i + 1} - Cute Cat HD Phone Background`,
}));

export default function CatGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `cat-wallpaper-${idx + 1}.png`;
      a.click();
