...
const ALL_IMAGES = Array.from({ length: 36 }, (_, i) => ({
  id: i,
  src: `/green-wallpapers/green-wallpaper${i + 1}.png`,
  alt: `Green Wallpaper ${i + 1} - Green Aesthetic Phone Background HD`,
}));

export default function GreenGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `green-wallpaper-${idx + 1}.png`;
      a.click();
