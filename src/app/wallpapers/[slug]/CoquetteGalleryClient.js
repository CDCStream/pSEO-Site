...
const ALL_IMAGES = Array.from({ length: 32 }, (_, i) => ({
  id: i,
  src: `/coquette-wallpapers/coquette-wallpaper${i + 1}.png`,
  alt: `Coquette Wallpaper ${i + 1} - Pink Bows Ribbons Vintage Pinterest Aesthetic Phone Background HD`,
}));

export default function CoquetteGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `coquette-wallpaper-${idx + 1}.png`;
      a.click();
