...
const ALL_IMAGES = Array.from({ length: 32 }, (_, i) => ({
  id: i,
  src: `/eddie-munson-wallpapers/eddie-munson-wallpaper${i + 1}.png`,
  alt: `Eddie Munson Wallpaper ${i + 1} - Stranger Things HD Phone Background`,
}));

export default function EddieMunsonGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `eddie-munson-wallpaper-${idx + 1}.png`;
      a.click();
