...
const ALL_IMAGES = Array.from({ length: 24 }, (_, i) => ({
  id: i,
  src: `/blue-wallpaper/blue-wallpaper${i + 1}.png`,
  alt: `Blue Wallpaper ${i + 1} - Aesthetic Blue Phone Background HD`,
}));

export default function BlueGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `blue-wallpaper-${idx + 1}.png`;
      a.click();
