...
const ALL_IMAGES = Array.from({ length: 32 }, (_, i) => ({
  id: i,
  src: `/thanksgiving-wallpapers/thanksgiving-wallpaper${i + 1}.png`,
  alt: `Thanksgiving Wallpaper ${i + 1} - Fall Harvest & Autumn Phone Background HD`,
}));

export default function ThanksgivingGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `thanksgiving-wallpaper-${idx + 1}.png`;
      a.click();
