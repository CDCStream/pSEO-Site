...
const ALL_IMAGES = Array.from({ length: 32 }, (_, i) => ({
  id: i,
  src: `/cool-wallpapers/cool-wallpaper${i + 1}.png`,
  alt: `Cool Wallpaper ${i + 1} - Aesthetic HD Phone Background for iPhone & Android`,
}));

export default function CoolGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `cool-wallpaper-${idx + 1}.png`;
      a.click();
