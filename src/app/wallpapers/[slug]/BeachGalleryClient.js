...
const ALL_IMAGES = Array.from({ length: 24 }, (_, i) => ({
  id: i,
  src: `/beach-wallpapers/beach-wallpaper${i + 1}.png`,
  alt: `Beach Wallpaper ${i + 1} - Free HD Tropical Ocean Phone Background`,
}));

export default function BeachGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `beach-wallpaper-${idx + 1}.png`;
      a.click();
        ...
        {ALL_IMAGES.map((img, i) => (
          ...
          >
            ...
            <button
              ...
              className="absolute bottom-2 right-2 p-2 bg-cyan-600 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity hover:bg-cyan-500"
              aria-label={`Download Beach Wallpaper ${i + 1}`}
            >
      ...
      {lightbox !== null && (
        <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center" onClick={closeLightbox}>
          ...
          <button
            ...
            className="absolute bottom-6 px-6 py-3 bg-cyan-600 hover:bg-cyan-500 rounded-xl text-white font-medium flex items-center gap-2 transition-colors"
          >
