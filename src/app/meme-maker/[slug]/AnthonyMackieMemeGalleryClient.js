...
const ALL_IMAGES = Array.from({ length: 16 }, (_, i) => ({
  id: i,
  src: `/anthony-mackie-memes/anthony-mackie-meme${i + 1}.gif`,
  ext: 'gif',
  alt: `Anthony Mackie Meme ${i + 1} - Funny Reaction GIF`,
}));

export default function AnthonyMackieMemeGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `anthony-mackie-meme-${idx + 1}.gif`;
      a.click();
