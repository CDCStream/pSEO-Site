...
const TEMPLATE_SRC = '/memes/get-a-load-of-this-guy-meme.jpg';
...
export default function GetALoadMemeClient() {
  ...
  const handleDownload = () => {
    ...
    link.download = 'get-a-load-of-this-guy-meme.png';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1">
            {textBoxes.map((box, i) => (
              <div
                key={box.id}
                className={`rounded-lg border transition-colors ${activeBox === box.id ? 'border-emerald-500/50 bg-emerald-500/10' : 'border-white/10 bg-white/5'}`}
              ...
              >
                <div className="flex items-center gap-2 p-2">
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="flex-1 px-2 py-1.5 rounded bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:border-emerald-500/50 placeholder-gray-600 resize-none leading-snug"
                  />
                  <button
                  ...
                  <div className="px-2 pb-2 pt-1 border-t border-white/5 space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <select
                        ...
                        className="flex-1 px-2 py-1 rounded bg-white/5 border border-white/10 text-white text-xs focus:outline-none focus:border-emerald-500/50 appearance-none cursor-pointer"
                      >
                        {FONTS.map((f) => (
                      ...
                      <div className="flex items-center gap-1 flex-wrap">
                        {PRESET_COLORS.map((c) => (
                          <button key={c} onClick={(e) => { e.stopPropagation(); updateBoxStyle(box.id, 'color', c); }}
                            className={`w-5 h-5 rounded-full border-2 transition-transform ${(box.color || '#ffffff') === c ? 'border-emerald-400 scale-125' : 'border-white/20'}`}
                      ...
                      <div className="flex items-center gap-1 flex-wrap">
                        {['#000000', '#ffffff', '#ff0000', '#0000ff', '#ffff00', 'none'].map((c) => (
                          <button key={c} onClick={(e) => { e.stopPropagation(); updateBoxStyle(box.id, 'stroke', c); }}
                            className={`w-5 h-5 rounded-full border-2 transition-transform ${(box.stroke || '#000000') === c ? 'border-emerald-400 scale-125' : 'border-white/20'}`}
          ...
          <button onClick={handleDownload}
            className="w-full py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-green-500 text-white font-semibold hover:opacity-90 transition-opacity mt-4 inline-flex items-center justify-center gap-2">
        ...
        <div>
          ...
          <div className="rounded-xl overflow-hidden border border-white/10 bg-black/20 cursor-crosshair select-none touch-none">
            <img ref={imgRef} src={TEMPLATE_SRC} alt="Get A Load Of This Guy Meme Template"
