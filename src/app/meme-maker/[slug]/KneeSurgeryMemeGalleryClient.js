...
const ALL_IMAGES = Array.from({ length: 8 }, (_, i) => ({
  id: i,
  src: `/knee-surgery-memes/knee-surgery-meme${i + 1}.gif`,
  alt: `Knee Surgery Meme ${i + 1} - Funny Knee Replacement Recovery Reaction GIF`,
}));

export default function KneeSurgeryMemeGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `knee-surgery-meme-${idx + 1}.gif`;
      a.click();
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
        {ALL_IMAGES.map((img, i) => (
          ...
          >
            ...
            />
            <span className="absolute top-2 left-2 px-1.5 py-0.5 bg-black/70 text-[10px] font-bold text-cyan-400 rounded uppercase tracking-wider">
              GIF
            </span>
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
            <button
              onClick={(e) => { e.stopPropagation(); handleDownload(img.src, i); }}
              className="absolute bottom-2 right-2 p-2 bg-cyan-500/90 hover:bg-cyan-400 text-white rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-300 backdrop-blur-sm"
        ...
        <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-sm flex items-center justify-center p-4" onClick={closeLightbox}>
          ...
          <div className="flex flex-col items-center gap-4" onClick={(e) => e.stopPropagation()}>
            ...
            <button
              ...
              className="flex items-center gap-2 px-6 py-3 bg-cyan-500 hover:bg-cyan-400 text-white rounded-xl font-semibold transition-colors"
            >
