...
const TEMPLATES = Array.from({ length: 7 }, (_, i) => ({
  id: i,
  src: `/dog-meme-templates/dog-meme-templates${i + 1}.png`,
  label: `Template ${i + 1}`,
}));
...
export default function DogMemeClient() {
  ...
  const drawMeme = useCallback(() => {
    ...
    // Draw position indicators for empty text boxes
    textBoxes.forEach((box) => {
      if (box.text) return;
      const cx = box.x * canvas.width;
      const cy = box.y * canvas.height;
  ...
  const handleCanvasClick = (e) => {
    ...
    // Check if clicking on existing text box to select it
  ...
  const handleDownload = () => {
    ...
    link.download = `dog-meme-${selected + 1}.png`;
        ...
        <div>
          ...
          >
            <img
              ...
              alt={`Dog Meme Template ${selected + 1}`}
