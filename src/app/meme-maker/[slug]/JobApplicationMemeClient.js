...
const TEMPLATES = Array.from({ length: 16 }, (_, i) => ({
  id: i,
  src: `/job-application-meme-templates/job-application-meme-template${i + 1}.png`,
  label: `Template ${i + 1}`,
}));
...
export default function JobApplicationMemeClient() {
  ...
  const handleDownload = () => {
    ...
    link.download = `job-application-meme-${selected + 1}.png`;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
            ...
            {textBoxes.map((box, i) => (
              <div
                ...
                className={`rounded-lg border transition-colors ${
                  activeBox === box.id ? 'border-orange-500/50 bg-orange-500/10' : 'border-white/10 bg-white/5'
                }`}
                ...
                {activeBox === box.id && (
                  <div className="px-2 pb-2 pt-1 border-t border-white/5 space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input type="range" min="12" max="72" step="2" value={box.fontSize || 36} onChange={(e) => updateBoxStyle(box.id, 'fontSize', Number(e.target.value))} className="flex-1 accent-orange-500 h-1.5" />
          ...
          <button
            ...
            className="w-full py-3 rounded-xl bg-gradient-to-r from-orange-500 to-amber-500 text-white font-semibold hover:opacity-90 transition-opacity mt-4 inline-flex items-center justify-center gap-2"
          >
        ...
        <div>
          ...
          <div ref={containerRef} className="rounded-xl overflow-hidden border border-white/10 bg-black/20 cursor-crosshair select-none touch-none">
            <img ref={imgRef} src={TEMPLATES[selected].src} alt={`Job Application Meme Template ${selected + 1}`} crossOrigin="anonymous" onLoad={drawMeme} className="hidden" />
            <canvas
              ref={canvasRef}
