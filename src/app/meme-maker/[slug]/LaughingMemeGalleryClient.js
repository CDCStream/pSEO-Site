...
const ALL_IMAGES = Array.from({ length: 22 }, (_, i) => ({
  id: i,
  src: `/laughing-memes/laughing-meme${i + 1}.gif`,
  alt: `Laughing Meme ${i + 1} - Funny Laughing Reaction GIF`,
}));

export default function LaughingMemeGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `laughing-meme-${idx + 1}.gif`;
      a.click();
