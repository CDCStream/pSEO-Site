...
const ALL_IMAGES = Array.from({ length: 23 }, (_, i) => ({
  id: i,
  src: `/crying-meme/crying-meme${i + 1}.png`,
  alt: `Crying Meme ${i + 1} - Funny Crying Reaction Meme HD`,
}));

export default function CryingMemeGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `crying-meme-${idx + 1}.png`;
      a.click();
