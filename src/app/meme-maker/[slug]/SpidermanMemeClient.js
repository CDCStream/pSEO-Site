...
const TEMPLATES = Array.from({ length: 14 }, (_, i) => ({
  id: i,
  src: `/spiderman-meme-templetes/spiderman-meme-templates${i + 1}.png`,
  label: `Template ${i + 1}`,
}));
...
export default function SpidermanMemeClient() {
  ...
  const handleDownload = () => {
    ...
    link.download = `spiderman-meme-${selected + 1}.png`;
    ...
    <div className="bg-white/5 rounded-2xl border border-white/10 p-4 sm:p-6">
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-7 gap-2">
          {TEMPLATES.map((tpl, i) => (
            <button
              key={tpl.id}
              onClick={() => { setSelected(i); setTextBoxes([]); setActiveBox(null); }}
              className={`relative rounded-lg overflow-hidden border-2 aspect-square transition-all ${selected === i ? 'border-red-500 ring-2 ring-red-500/30 scale-105' : 'border-white/10 hover:border-white/30'}`}
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          {textBoxes.length === 0 && (
            <div className="text-center py-6 text-gray-600 text-sm">No text boxes yet. Click on the image to add one.</div>
          )}
          ...
          <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1">
            {textBoxes.map((box, i) => (
              <div
                key={box.id}
                className={`rounded-lg border transition-colors ${activeBox === box.id ? 'border-red-500/50 bg-red-500/10' : 'border-white/10 bg-white/5'}`}
              ...
              >
                <div className="flex items-center gap-2 p-2">
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="flex-1 px-2 py-1.5 rounded bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:border-red-500/50 placeholder-gray-600 resize-none leading-snug"
                  />
                  <button
                  ...
                  <div className="px-2 pb-2 pt-1 border-t border-white/5 space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <select
                        ...
                        className="flex-1 px-2 py-1 rounded bg-white/5 border border-white/10 text-white text-xs focus:outline-none focus:border-red-500/50 appearance-none cursor-pointer"
                      >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input type="range" min="12" max="72" step="2" value={box.fontSize || 36}
                      ...
                      <div className="flex items-center gap-1 flex-wrap">
                        {PRESET_COLORS.map((c) => (
                          <button key={c} onClick={(e) => { e.stopPropagation(); updateBoxStyle(box.id, 'color', c); }}
                            className={`w-5 h-5 rounded-full border-2 transition-transform ${(box.color || '#ffffff') === c ? 'border-red-400 scale-125' : 'border-white/20'}`}
                      ...
                      <div className="flex items-center gap-1 flex-wrap">
                        {['#000000', '#ffffff', '#ff0000', '#0000ff', '#ffff00', 'none'].map((c) => (
                          <button key={c} onClick={(e) => { e.stopPropagation(); updateBoxStyle(box.id, 'stroke', c); }}
                            className={`w-5 h-5 rounded-full border-2 transition-transform ${(box.stroke || '#000000') === c ? 'border-red-400 scale-125' : 'border-white/20'}`}
                        ...
                        <input type="color" value={box.stroke === 'none' ? '#000000' : (box.stroke || '#000000')}
                          onChange={(e) => updateBoxStyle(box.id, 'stroke', e.target.value)}
                          className="w-5 h-5 rounded cursor-pointer border-0 bg-transparent" title="Custom stroke color" />
                      </div>
                    </div>
                  </div>
                )}
        ...
        <div>
          ...
          <div className="rounded-xl overflow-hidden border border-white/10 bg-black/20 cursor-crosshair select-none touch-none">
            <img ref={imgRef} src={TEMPLATES[selected].src} alt={`Spiderman Meme Template ${selected + 1}`}
              crossOrigin="anonymous" onLoad={drawMeme} className="hidden" />
