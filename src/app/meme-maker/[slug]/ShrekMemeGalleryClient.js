...
const ALL_IMAGES = Array.from({ length: 18 }, (_, i) => ({
  id: i,
  src: `/shrek-memes/shrek-meme${i + 1}.gif`,
  ext: 'gif',
  alt: `Shrek Meme ${i + 1} - Funny Shrek Reaction GIF`,
}));

export default function ShrekMemeGalleryClient() {
  ...
  const handleDownload = async (src, idx) => {
    try {
      ...
      a.href = url;
      a.download = `shrek-meme-${idx + 1}.gif`;
      a.click();
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {ALL_IMAGES.map((img, i) => (
          ...
          >
            ...
            />
            <span className="absolute top-2 left-2 px-1.5 py-0.5 bg-black/70 text-[10px] font-bold text-lime-400 rounded uppercase tracking-wider">
              GIF
            </span>
