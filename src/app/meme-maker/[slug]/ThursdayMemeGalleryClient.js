...
const ALL_IMAGES = Array.from({ length: 21 }, (_, i) => ({
  id: i,
  src: `/thursday-memes/thursday-meme${i + 1}.gif`,
  ext: 'gif',
  alt: `Thursday Meme ${i + 1} - Funny Happy Thursday Reaction GIF`,
}));

export default function ThursdayMemeGalleryClient() {
  ...
  const handleDownload = async (src, idx, ext) => {
    try {
      ...
      a.href = url;
      a.download = `thursday-meme-${idx + 1}.${ext}`;
      a.click();
