...
const ALL_GIFS = Array.from({ length: 15 }, (_, i) => ({
  id: i + 1,
  src: `/airball-memes/airball-meme${i + 1}.gif`,
  alt: `Airball meme GIF #${i + 1} \u2014 basketball airball reaction, free download`,
}));

export default function AirballMemeGalleryClient() {
  ...
  const handleDownload = useCallback(
    async (src, idx) => {
      try {
        ...
        a.href = url;
        a.download = `airball-meme-${idx + 1}.gif`;
        document.body.appendChild(a);
        a.click();
        a.remove();
        URL.revokeObjectURL(url);
        addToast('Airball meme GIF downloaded!', 'success');
      } catch {
  ...
  return (
    <>
      <div className="mb-4 flex items-center gap-2 text-xs text-gray-400">
        <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-full bg-orange-500/10 border border-orange-500/30 text-orange-200 font-medium">
        ...
        {ALL_GIFS.map((gif, i) => (
          ...
          >
            ...
            <button
              ...
              className="absolute bottom-2 right-2 inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-orange-500/95 hover:bg-orange-400 text-white text-xs font-semibold rounded-lg opacity-0 group-hover:opacity-100 focus:opacity-100 transition-all duration-300 backdrop-blur-sm shadow-lg"
          ...
          <div className="flex flex-col items-center gap-4 max-w-3xl w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-6 py-3 bg-orange-500 hover:bg-orange-400 text-white rounded-xl font-semibold transition-colors shadow-lg"
              >
