/**
 * One-shot script: optimize public/kpop-demon-hunters-wallpapers/*.png
 * in place using palette-quantised PNG. Same approach as the Peppa Pig
 * gallery optimiser — adaptive 256-colour palette via libimagequant
 * gives a 3-5x size reduction on stylised anime-style 3D renders with
 * limited colour count, which is the SEO-health-score lever (Core Web
 * Vitals LCP) for an image-heavy gallery page.
 *
 * Run: node scripts/optimize-kpop-demon-hunters-wallpapers.js
 */
const sharp = require('sharp');
const fs = require('fs');
const path = require('path');

const DIR = path.join(__dirname, '..', 'public', 'kpop-demon-hunters-wallpapers');

async function main() {
  const files = fs
    .readdirSync(DIR)
    .filter((f) => /^kpop-demon-hunters-wallpaper\d+\.png$/i.test(f))
    .sort((a, b) => {
